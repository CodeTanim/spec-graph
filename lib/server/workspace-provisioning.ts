...
import type { SpecGraphIdentity } from "../auth/current-user";
import { ApiError } from "./http";

export type WorkspaceContext = {
  user: SpecGraphIdentity & { databaseId: string };
  workspace: {
    id: string;
    name: string;
  };
...
export async function ensureWorkspaceForUser(
  ...
  if (!databaseUser) {
    throw new ApiError(
      500,
      "USER_SETUP_FAILED",
      "SpecGraph could not prepare your account.",
    );
  }
