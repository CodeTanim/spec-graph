...
export async function getConfluenceConnectionSession(
  ...
  try {
    const context: ConfluenceSessionContext = session.contextJson
      ? JSON.parse(session.contextJson) as ConfluenceSessionContext
      : {};
    let sourceGroupId = context.sourceGroupId || null;
    if (!sourceGroupId && context.repositorySourceId) {
      sourceGroupId = await sourceGroupIdForSource(
        workspaceId,
        context.repositorySourceId,
        db,
      );
    }
    return {
